...
import {
  conexaoPorToken, registrarLogWebhook, ensureWebhookEntradaSchema,
...
/**
 * Receptor de webhook de entrada de lead — um token POR WEBHOOK.
 *
 * O gestor cria o webhook na aba Integrações, dá um nome a ele ("Datalytics",
 * "Formulário do site"…) e cola a URL gerada no sistema de origem. Um cliente
 * pode ter quantos quiser; o token resolve qual é.
 *
 * ⚠️ Este handler é servido por DOIS caminhos: `/api/integrations/webhook/` e
 * `/api/integrations/datalytics/`. O segundo é o endereço original e continua
 * valendo PARA SEMPRE — há URL desse formato colada no painel de cliente em
 * produção, e o dia em que ela responder 404 os leads param de entrar sem
 * ninguém ser avisado. Alias não é dívida: é o contrato que já foi publicado.
 *
 * Um único handler trata tudo (etapa presente = mudança de etapa; ausente =
 * lead novo), porque o shape do payload varia por origem e o campo de evento
 * pode nem vir.
 *
 * As rotas são PUBLIC_PREFIX no proxy — o token de 48 hex é a credencial.
 *
 * Filosofia de resposta (espelha o webhook genérico): payload ruim → 200 com
 * ok:false (retry não conserta payload; a política de retry da origem é
 * desconhecida e o Cardápio Web ensina que provedores PAUSAM webhook após N
 * falhas). Erro NOSSO (banco) → 500, retry bem-vindo. Tudo vai pro log com o
 * payload CRU — é como se descobre o shape real de uma origem nova.
 */
 ...
 * etapa que só existe no CRM de origem ("Follow 2" etc.).
 */
...
async function upsertLead(
  ...
  // criado" e o "Etapa atualizada" que a origem manda quase juntos.
  ...
  try {
    ...
    if (match) {
      ...
      await pool.query(
        `UPDATE public.crm_leads SET
           -- Etapa: quem manda o webhook é a fonte de verdade — SOBRESCREVE
           -- (exceção deliberada ao fill-blanks; sem etapa, o status fica).
    ...
    const { rows: [novo] } = await pool.query<{ id: string }>(
      ...
      [
        ...
        lead.nome,
        lead.telefoneBruto,
        // ⚠️ `canal` e `origin` NÃO carregam o nome do webhook, de propósito.
        // Estes dois literais são PORTA DE ENTRADA, não canal: `canalSql()`,
        // `lead-identity` e `origem-digital` descartam 'datalytics' justamente
        // para que o canal real venha do rastreio do payload (UTM, click id).
        // Gravar aqui "Formulário do site" faria o nome de um webhook virar
        // uma fatia no donut de canais de todo cliente que criasse um.
        'datalytics',
...
export async function receberWebhook(req: NextRequest, token: string) {
  ...
  try {
    ...
    if (!conn) {
      await registrarLogWebhook(pool, { clientId: null, raw, resultado: 'token_invalido' });
      return Response.json({ ok: false, erro: 'token_invalido' }, { status: 401 });
    }
    if (!conn.enabled) {
      await registrarLogWebhook(pool, {
        clientId: conn.client_id, conexaoId: conn.id, raw, resultado: 'desativado',
      });
    ...
    await recordTrackingEvent(pool, {
      ...
      regiaoUf: lead.estado ?? regiao?.uf ?? null,
      raw,
    }).catch(err => console.error('[webhook-entrada] tracking event', err));
    ...
    console.error('[webhook-entrada] erro na recepção', err);
    await registrarLogWebhook(pool, {
      clientId: null, raw: { erro: true },
      resultado: 'erro', detalhe: err instanceof Error ? err.message : String(err),
    }).catch(() => {});
    // 500 de propósito: erro NOSSO — se a origem tiver retry, é bem-vindo.
...
export async function testarWebhook(token: string) {
  const pool = makeServerPool();
  try {
    ...
    await registrarLogWebhook(pool, {
      clientId: conn.client_id, conexaoId: conn.id, raw: { teste: 'GET' }, resultado: 'teste_get',
    });
    return Response.json({
      ok: true, integracao: 'webhook', webhook: conn.nome,
      cliente: conn.client_id, ativa: conn.enabled,
    });
  } finally {
    await pool.end();
  }
