...
import { compilarResumoDoDia } from '@/lib/resumo-dia';
  ...
  try {
    ...
    for (const cliente of alvo) {
      ...
      try {
        ...
        // Corte exato pro dia-alvo pelo prefixo (cobre "T" e espaço).
        const doDia = [...meta, ...google].filter(ev => ev.criadoEm.slice(0, 10) === dia);
