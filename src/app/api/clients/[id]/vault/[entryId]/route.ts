...
export async function PUT(
  ...
  // Senha crua vinda da tela vira texto cifrado antes do UPDATE.
  ...
  try {
    const { rows } = await pool.query(
      ...
      [body.title.trim(), body.url ?? null, body.login ?? null, senha.valor,
       body.category ?? 'Outros', body.notes ?? null, entryId, clientId]
    );
    if (!rows[0]) return Response.json({ error: 'Not found' }, { status: 404 });
    return Response.json(linhaVaultPublica(rows[0]));
  } finally {
    await pool.end();
  }
}
...
export async function DELETE(
  ...
  if (!getSession(_req)) return unauthorized();
  const { id: clientId, entryId } = await params;
  const pool = makeServerPool();
  try {
    await pool.query(
