...
import { linhaVaultPublica, prepararParaGravar } from '@/lib/vault-crypto';
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // O campo chega com a senha CRUA digitada na tela (o nome `password_enc` é
  // herança do banco). Ela é cifrada aqui, antes de qualquer INSERT.
  const senha = prepararParaGravar(body.password_enc);
  if (!senha.ok) return Response.json({ error: senha.motivo }, { status: 503 });
  ...
  try {
    ...
    const { rows } = await pool.query(
      ...
      [clientId, body.title.trim(), body.url ?? null, body.login ?? null,
       senha.valor, body.category ?? 'Outros', body.notes ?? null]
    );
    return Response.json(linhaVaultPublica(rows[0]), { status: 201 });
  } finally {
    await pool.end();
  }
}
