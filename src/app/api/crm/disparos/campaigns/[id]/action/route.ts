...
export async function POST(
  ...
  try {
    ...
    if (action === 'pause') {
      await pool.query(`UPDATE public.crm_disparo_campaigns SET status = 'paused' WHERE id = $1`, [id]);
      ...
      await pool.query(`UPDATE public.crm_disparo_campaigns SET status = 'cancelled' WHERE id = $1`, [id]);
    }

    const { rows: [updated] } = await pool.query(
      `SELECT id, status, sent, failed, total FROM public.crm_disparo_campaigns WHERE id = $1`,
      [id],
    );
