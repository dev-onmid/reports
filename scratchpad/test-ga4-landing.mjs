...
import { parseTotais, parseEventos, parseOrigens, parseLinhas, parseDiario, parseSeg, parseSemanaHora, parseFunil, ordenaRolagem, somaSeg, consolidar, faixasDoPeriodo, totaisVazios } from './build-ga4/ga4-landing.mjs';
...
const rel = (id, wa, sess) => ({
  ...
  pago: {
    canais: [{ valor: 'Paid Search', sessoes: sess, engajadas: sess / 2, tempo: sess * 30, conversoes: wa, sessoesConv: wa }],
    campanhas: [], palavras: [], termos: [],
    googleAds: [{ valor: 'Pesquisa', sessoes: sess, engajadas: 0, tempo: 0, conversoes: wa, sessoesConv: wa, custo: 100, cliques: sess }],
  },
  audiencia: { dispositivos: [{ valor: 'mobile', sessoes: sess, engajadas: 0, tempo: 0, conversoes: wa, sessoesConv: wa }], cidades: [], novosRecorrentes: [], idades: [], generos: [], semanaHora: [{ dia: 1, hora: 10, sessoes: sess, conversoes: wa }] },
...
t('parseSeg: 2ª dimensão vira sub, métricas na ordem sessions/engaged/duração/keyEvents', () => {
  ...
  assert.deepEqual(r, [{ valor: '[ON] Pesquisa', sub: 'google / cpc', sessoes: 100, engajadas: 60, tempo: 3000, conversoes: 12, sessoesConv: 8 }]);
});
...
t('somaSeg junta pelo par valor+sub e soma custo só onde existe', () => {
  const r = somaSeg([[{ valor: 'A', sessoes: 1, engajadas: 1, tempo: 10, conversoes: 1, sessoesConv: 1, custo: 5 }], [{ valor: 'A', sessoes: 2, engajadas: 0, tempo: 5, conversoes: 0, sessoesConv: 0, custo: 7 }, { valor: 'A', sub: 'x', sessoes: 9, engajadas: 0, tempo: 0, conversoes: 0, sessoesConv: 0 }]]);
  assert.equal(r[0].sub, 'x'); assert.equal(r[1].sessoes, 3); assert.equal(r[1].custo, 12); assert.equal(r[1].sessoesConv, 1); assert.equal(r[0].custo, undefined);
});
